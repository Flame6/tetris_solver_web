...
CASES = {
    ...
    'docs': {'name': 'Documents Case', 'width': 1, 'height': 2},
    ...
    'Twitch': {'name': 'Twitch Case', 'width': 3, 'height': 3}  # New case added here
}

def read_counter():
    """Reads the current count from the counter.txt file"""
    try:
...
def update_counter():
    """Updates the counter in the counter.txt file"""
...
def greedy_solver(cases):
    ...
    sorted_cases = sorted(cases.items(), key=lambda item: (CASES[item[0]]['width'] * CASES[item[0]]['height']), reverse=True)

    for label, quantity in sorted_cases:
        block = CASES[label]
...
def index():
    ...
    count = read_counter()  # Read the current count
    return render_template('index.html', cases=CASES, count=count)
